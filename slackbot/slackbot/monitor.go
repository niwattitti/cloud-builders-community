...
func Monitor(ctx context.Context, title string, icon string, tag string, projectId string, buildId string, webhook string) {
	svc := gcbClient(ctx)
	errors := 0
	...
	for {
		...
		case "SUCCESS", "FAILURE", "INTERNAL_ERROR", "TIMEOUT", "CANCELLED":
			...
			Notify(monitoredBuild, title, icon, tag, webhook)
			return
		}
		<-t
	}
